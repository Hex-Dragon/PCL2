...
#Region "不同类型整合包的安装方法"
    ...
    Private Sub InstallPackCurseForge(FileAddress As String, Archive As Compression.ZipArchive, ArchiveBaseFolder As String, Optional VersionName As String = Nothing, Optional Logo As String = Nothing)
        ...
        If VersionName Is Nothing Then
            ...
            VersionName = MyMsgBoxInput(GetLang("LangModModpackInputInstanceName"), "", PackName, New ObjectModel.Collection(Of Validate) From {Validate})
    ...
    Private Sub InstallPackHMCL(FileAddress As String, Archive As Compression.ZipArchive, ArchiveBaseFolder As String)
        ...
        Dim Validate As New ValidateFolderName(PathMcFolder & "versions")
        If Validate.Validate(PackName) <> "" Then PackName = ""
        Dim VersionName As String = MyMsgBoxInput(GetLang("LangModModpackInputInstanceName"), "", PackName, New ObjectModel.Collection(Of Validate) From {Validate})
    ...
    Private Sub InstallPackCompress(FileAddress As String, ArchiveBaseFolder As String)
        ...
        If TargetFolder.Contains("!") OrElse TargetFolder.Contains(";") Then Hint(GetLang("LangModModpackFolderNoExclamationOrSemicolon"), HintType.Critical) : Exit Sub
        If Directory.GetFileSystemEntries(TargetFolder).Length > 0 Then Hint(GetLang("LangModModpackFolderShouldBeEmpty"), HintType.Critical) : Exit Sub
        '要求显示名称
        ...
        Hint(GetLang("LangModModpackAddedToGameList", NewName), HintType.Finish)
    End Sub
