...
#Region "初始化"
    ...
    Public Sub New(ResourceType As CompType)
...
#Region "UI 化"
    ...
    Private Sub LoadUIFromLoaderOutput()
        Try
            ...
            '修改缓存
            ModItems.Clear()
            For Each ModEntity As McComp In McModLoader.Output
    ...
    Private Sub McModContent(sender As MyLocalModItem, e As EventArgs)
        ...
        If sender.Entry.State = McComp.McModState.Unavailable Then
    ...
    Public Sub RefreshUI()
        ...
        Dim ShowingMods = If(IsSearching, SearchResult, If(McModLoader.Output, New List(Of McComp))).Where(Function(m) CanPassFilter(m)).ToList
        '重新列出列表
        AniControlEnabled += 1
    ...
    Public Sub RefreshBars()
        ...
        For Each ModItem In If(IsSearching, SearchResult, If(McModLoader.Output, New List(Of McComp)))
            AnyCount += 1
...
#Region "下边栏"
    ...
    Private Sub BtnSelectUpdate_Click() Handles BtnSelectUpdate.Click
        Dim UpdateList As List(Of McComp) = McModLoader.Output.Where(Function(m) SelectedMods.Contains(m.RawFileName) AndAlso m.CanUpdate).ToList()
    ...
    Public Sub UpdateMods(ModList As IEnumerable(Of McComp))
        ...
        Try
            ...
            End Sub))
            '结束处理
            Dim Loader As New LoaderCombo(Of IEnumerable(Of McComp))("Mod 更新：" & PageVersionLeft.Version.Name, InstallLoaders)
...
#Region "搜索"
    ...
    Public Sub SearchRun() Handles SearchBox.TextChanged
        If IsSearching Then
            ...
            For Each Entry As McComp In McModLoader.Output
