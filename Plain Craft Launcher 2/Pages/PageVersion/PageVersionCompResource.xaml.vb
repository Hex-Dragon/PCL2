...
#Region "UI 化"
    ...
    Private Sub LoadUIFromLoaderOutput()
        Try
            ...
            '修改缓存
            ModItems.Clear()
            For Each ModEntity As ModLocalCompFile In CompModLoader.Output
        ...
        End Try
    End Sub
    Private Function BuildLocalCompItem(Entry As ModLocalCompFile) As MyLocalModItem
        AniControlEnabled += 1
    ...
    Private Sub BuildLocalCompItemBtnHandler(sender As MyLocalModItem, e As EventArgs)
        ...
        Dim BtnED As New MyIconButton With {.LogoScale = 1, .Logo = If(sender.Entry.State = ModLocalCompFile.McModState.Fine, Logo.IconButtonStop, Logo.IconButtonCheck), .Tag = sender}
        BtnED.ToolTip = If(sender.Entry.State = ModLocalCompFile.McModState.Fine, "禁用", "启用")
        ...
        If sender.Entry.State = ModLocalCompFile.McModState.Unavailable Then
    ...
    Public Sub RefreshUI()
        ...
        Dim ShowingMods = If(IsSearching, SearchResult, If(CompModLoader.Output, New List(Of ModLocalCompFile))).Where(Function(m) CanPassFilter(m)).ToList
        '重新列出列表
        AniControlEnabled += 1
    ...
    Public Sub RefreshBars()
        ...
        For Each ModItem In If(IsSearching, SearchResult, If(CompModLoader.Output, New List(Of ModLocalCompFile)))
            AnyCount += 1
            If ModItem.CanUpdate Then UpdateCount += 1
            If ModItem.State.Equals(ModLocalCompFile.McModState.Fine) Then EnabledCount += 1
            If ModItem.State.Equals(ModLocalCompFile.McModState.Disabled) Then DisabledCount += 1
            If ModItem.State.Equals(ModLocalCompFile.McModState.Unavailable) Then UnavalialeCount += 1
        Next
        '显示
        ...
        If Selected Then
            ...
            For Each ModEntity In CompModLoader.Output
                If SelectedMods.Contains(ModEntity.RawFileName) Then
                    ...
                    If ModEntity.State = ModLocalCompFile.McModState.Fine Then
                        HasEnabled = True
                    ElseIf ModEntity.State = ModLocalCompFile.McModState.Disabled Then
                        HasDisabled = True
                    End If
                End If
            Next
...
#Region "筛选"
    ...
    ''' </summary>
    Private Function CanPassFilter(CheckingMod As ModLocalCompFile) As Boolean
        Select Case Filter
            Case FilterType.All
                Return True
            Case FilterType.Enabled
                Return CheckingMod.State = ModLocalCompFile.McModState.Fine
            Case FilterType.Disabled
                Return CheckingMod.State = ModLocalCompFile.McModState.Disabled
            Case FilterType.CanUpdate
                Return CheckingMod.CanUpdate
            Case FilterType.Unavailable
                Return CheckingMod.State = ModLocalCompFile.McModState.Unavailable
            Case Else
                Return False
        End Select
    End Function
...
#Region "排序"
    ...
    Private Function GetSortMethod(Method As SortMethod) As Func(Of ModLocalCompFile, ModLocalCompFile, Integer)
        Select Case Method
            Case SortMethod.FileName
                Return Function(a As ModLocalCompFile, b As ModLocalCompFile) As Integer
...
#Region "下边栏"
    ...
    Private Sub BtnSelectED_Click(sender As MyIconTextButton, e As RouteEventArgs) Handles BtnSelectEnable.Click, BtnSelectDisable.Click
        ...
        ChangeAllSelected(False)
    End Sub
    Private Sub EDMods(ModList As IEnumerable(Of ModLocalCompFile), IsEnable As Boolean)
        ...
        For Each ModE In ModList.ToList
            ...
            If ModEntity.State = ModLocalCompFile.McModState.Fine AndAlso Not IsEnable Then
                '禁用
                NewPath = ModEntity.Path & If(File.Exists(ModEntity.Path & ".old"), ".old", ".disabled")
            ElseIf ModEntity.State = ModLocalCompFile.McModState.Disabled AndAlso IsEnable Then
                '启用
            ...
            End Try
            '更改 Loader 中的列表
            Dim NewModEntity As New ModLocalCompFile(NewPath, CurrentCompType)
    ...
    Private Sub BtnSelectUpdate_Click() Handles BtnSelectUpdate.Click
        Dim UpdateList As List(Of ModLocalCompFile) = CompModLoader.Output.Where(Function(m) SelectedMods.Contains(m.RawFileName) AndAlso m.CanUpdate).ToList()
    ...
    Public Sub UpdateResource(ModList As IEnumerable(Of ModLocalCompFile))
        '更新前警告
        ...
        Try
            ...
            For Each Entry As ModLocalCompFile In ModList
            ...
            End Sub))
            '结束处理
            Dim Loader As New LoaderCombo(Of IEnumerable(Of ModLocalCompFile))("资源更新：" & PageVersionLeft.Version.Name, InstallLoaders)
    ...
    Private Sub BtnSelectDelete_Click() Handles BtnSelectDelete.Click
        ...
        ChangeAllSelected(False)
    End Sub
    Private Sub DeleteMods(ModList As IEnumerable(Of ModLocalCompFile))
        Try
            Dim IsSuccessful As Boolean = True
            Dim IsShiftPressed As Boolean = My.Computer.Keyboard.ShiftKeyDown
            '确认需要删除的文件
            ModList = ModList.SelectMany(
            Function(Target As ModLocalCompFile)
                If Target.State = ModLocalCompFile.McModState.Fine Then
                    Return {Target.Path, Target.Path & If(File.Exists(Target.Path & ".old"), ".old", ".disabled")}
                Else
                    Return {Target.Path, Target.RawPath}
                End If
            End Function).Distinct.Where(Function(m) File.Exists(m)).Select(Function(m) New ModLocalCompFile(m, CurrentCompType)).ToList()
            '实际删除文件
...
#Region "单个 Mod 项"
    ...
    Public Sub Info_Click(sender As Object, e As EventArgs)
        Try
            ...
            Dim ModEntry As ModLocalCompFile = CType(If(TypeOf sender Is MyIconButton, sender.Tag, sender), MyLocalModItem).Entry
            '加载失败信息
            If ModEntry.State = ModLocalCompFile.McModState.Unavailable Then
    ...
    Public Sub ED_Click(sender As MyIconButton, e As EventArgs)
        Dim ListItem As MyLocalModItem = sender.Tag
        EDMods({ListItem.Entry}, ListItem.Entry.State = ModLocalCompFile.McModState.Disabled)
    End Sub
...
#Region "搜索"
    ...
    Public ReadOnly Property IsSearching As Boolean
        ...
        End Get
    End Property
    Private SearchResult As List(Of ModLocalCompFile)

    Public Sub SearchRun() Handles SearchBox.TextChanged
        If IsSearching Then
            '构造请求
            Dim QueryList As New List(Of SearchEntry(Of ModLocalCompFile))
            For Each Entry As ModLocalCompFile In CompModLoader.Output
                ...
                End If
                QueryList.Add(New SearchEntry(Of ModLocalCompFile) With {.Item = Entry, .SearchSource = SearchSource})
            Next
            '进行搜索
