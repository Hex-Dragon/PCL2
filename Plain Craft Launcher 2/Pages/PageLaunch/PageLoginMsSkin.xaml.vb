...
#Region "皮肤/披风"
    ...
    Public Sub BtnSkinEdit_Click(sender As Object, e As RoutedEventArgs)
        '检查条件，获取新皮肤
        If IsChanging Then
            Hint(Application.Current.FindResource("LangPageLoginMsSkinChangingSkinPlzWait"))
            Exit Sub
        End If
        If McLoginLoader.State = LoadState.Failed Then
            Hint(Application.Current.FindResource("LangPageLoginMsSkinChangeSkinFailByLoginFail"), HintType.Critical)
            Exit Sub
        End If
        Dim SkinInfo As McSkinInfo = McSkinSelect()
        If Not SkinInfo.IsVaild Then Exit Sub
        Hint(Application.Current.FindResource("LangPageLoginMsSkinChangingSkin"))
        IsChanging = True
        '开始实际获取
        RunInNewThread(
        Async Sub()
...
Retry:
                ...
                If Result.Contains("request requires user authentication") Then
                    Hint(Application.Current.FindResource("LangPageLoginMsSkinLoginBeforeChangeSkin"))
                    McLoginMsLoader.Start(GetLoginData(), IsForceRestart:=True)
                    GoTo Retry
                ElseIf Result.Contains("""error""") Then
                    Hint(Application.Current.FindResource("LangPageLoginMsSkinChangeSkinFail") & ":" & GetJson(Result)("error"), HintType.Critical)
                    Exit Sub
                End If
                '获取新皮肤地址
            ...
            Catch ex As Exception
                If ex.GetType.Equals(GetType(Tasks.TaskCanceledException)) Then
                    Hint(Application.Current.FindResource("LangPageLoginMsSkinChangeSkinFailByTimeOut"), HintType.Critical)
                Else
                    Log(ex, Application.Current.FindResource("LangPageLoginMsSkinChangeSkinFail"), LogLevel.Hint)
                End If
            Finally
                IsChanging = False
            End Try
