    ...
    Private Sub WorkThread()
        ...
        Do While Not MyConverter.IsExited
            Try
                ...
                '检查轮询验证地址
                If OAuthUrl = "" Or OAuthUrl Is Nothing Then Throw New Exception("轮询验证地址为空")
                Dim ClientID As String
                If OAuthUrl.ToLower.Contains("login.microsoftonline.com") Then
                ...
                '获取结果
                Dim ResultJson As JObject = GetJson(Result.ToString)
            ...
            Catch ex As Exception
                ...
                ElseIf ex.Message.Contains("authorization_pending") Then
                    Thread.Sleep(2000)
                ElseIf ex.Message.Contains("slow_down") Then
                    Log(ex, $"登录轮询速率过快")
                    Thread.Sleep(2000)
                ElseIf UnknownFailureCount <= 2 Then
                    ...
                    Thread.Sleep(2000)
                ElseIf ex.Message.Contains("轮询验证地址为空") Then
                    Log(ex, "登录轮询失败"， LogLevel.Feedback)
                Else
