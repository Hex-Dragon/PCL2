Public Class PageLinkLeft
    ...
    Private Sub PageLinkLeft_Loaded(sender As Object, e As RoutedEventArgs) Handles Me.Loaded
        ...
        ItemDirect.SetChecked(True, False, False)
    End Sub
...
#Region "页面切换"
    ...
    ''' 当前页面的编号。
    ''' </summary>
    Public PageID As FormMain.PageSubType = FormMain.PageSubType.LinkDirect

    ''' <summary>
    ''' 勾选事件改变页面。
    ''' </summary>
    Private Sub PageCheck(sender As MyListItem, e As RouteEventArgs) Handles ItemDirect.Check, ItemSetup.Check, ItemHelp.Check, ItemFeedback.Check, ItemNetStatus.Check
    ...
    Public Function PageGet(Optional ID As FormMain.PageSubType = -1)
        ...
        Select Case ID
            Case 0, FormMain.PageSubType.LinkDirect
                If FrmLinkDirect Is Nothing Then FrmLinkDirect = New PageLinkDirect
                Return FrmLinkDirect
