Public Class PageSelectLeft
    ...
    Private Sub McFolderListUI()
        Try
            ...
            For Each Folder As McFolder In McFolderList.ToArray
                ...
                End Select
                If (Folder.Type = McFolderType.Original OrElse Folder.Type = McFolderType.RenamedOriginal) AndAlso Folder.Path = Path & ".minecraft\" AndAlso McFolderList.Count = 1 Then CType(ContMenu.FindName("Delete"), MyMenuItem).Header = Application.Current.FindResource("LangSelectEmpty")
                '注册事件
            ...
            '标题文本
            FrmSelectLeft.PanList.Children.Add(New TextBlock With {.Text = Application.Current.FindResource("LangSelectAddFolder"), .Margin = New Thickness(13, 18, 5, 4), .Opacity = 0.6, .FontSize = 12})

            '确认创建按钮状态
            If Not Directory.Exists(Path & ".minecraft\") Then
                Dim ItemCreate As New MyListItem With {.IsScaleAnimationEnabled = False, .Type = MyListItem.CheckType.Clickable, .Title = Application.Current.FindResource("LangSelectCreateFolder"), .Height = 34,
                    .ToolTip = Application.Current.FindResource("LangSelectCreateFolderToolTip"),
                    .LogoScale = 0.9,
            ...
            '添加按钮
            Dim ItemAdd As New MyListItem With {.IsScaleAnimationEnabled = False, .Type = MyListItem.CheckType.Clickable, .Title = Application.Current.FindResource("LangSelectAddExistFolder"), .Height = 34,
                .ToolTip = Application.Current.FindResource("LangSelectAddExistFolderToolTip"),
            ...
            '安装按钮
            Dim ItemInstall As New MyListItem With {.IsScaleAnimationEnabled = False, .Type = MyListItem.CheckType.Clickable, .Title = Application.Current.FindResource("LangSelectAddModpackages"), .Height = 34,
                .ToolTip = Application.Current.FindResource("LangSelectAddModpackagesToolTip"),
            ...
            If Not McFolderList.Any() Then
                Throw New ArgumentNullException(Application.Current.FindResource("LangSelectNoMCFolder"))
            Else
        ...
        Catch ex As Exception
            Log(ex, Application.Current.FindResource("LangSelectUIError"), LogLevel.Feedback)
        Finally
    ...
    Public Sub Add_Click()
        ...
        If HasDownloadingTask() Then
            Hint(Application.Current.FindResource("LangSelectDownloadingTaskExistsNoAdd"), HintType.Critical)
            Exit Sub
        End If
        Try
            '获取输入
            ...
            If NewFolder.Contains("!") OrElse NewFolder.Contains(";") Then Hint(Application.Current.FindResource("LangSelectNoSpecialSymbol"), HintType.Critical) : Exit Sub
            '要求输入显示名称
            ...
            Dim NewName As String = MyMsgBoxInput(Application.Current.FindResource("LangSelectDialogInputNameTitle"), Application.Current.FindResource("LangSelectDialogInputNameContent"), DefaultName,
        ...
        Catch ex As Exception
            Log(ex, String.Format(Application.Current.FindResource("LangSelectFailToAddFolder"), NewFolder), LogLevel.Feedback)
        End Try
    End Sub
        ...
        Sub()
            Try
                ...
                If Not CheckPermission(FolderPath) Then
                    If ShowHint Then
                        Hint(Application.Current.FindResource("LangSelectNoPermission"), HintType.Critical)
                        Exit Sub
                    Else
                ...
                For i = 0 To Folders.Count - 1
                    ...
                    If Folder.Split(">")(1) = FolderPath Then
                        ...
                        If Folder.Split(">")(0) = DisplayName Then
                            If ShowHint Then Hint(Application.Current.FindResource("LangSelectAlreadyAdded"), HintType.Info)
                            Exit Sub
                        Else
                            Folders(i) = DisplayName & ">" & FolderPath
                            IsReplace = True
                            If ShowHint Then Hint(String.Format(Application.Current.FindResource("LangSelectRenameSuccess"), DisplayName), HintType.Finish)
                        End If
                        Exit For
                    End If
                Next
                ...
                If ShowHint Then Hint(String.Format(Application.Current.FindResource("LangSelectFolderAdded"), DisplayName), HintType.Finish)
                '自动关闭版本隔离
                '1. 根目录中存在数个 Mod
                ...
                Next
            Catch ex As Exception
                Log(ex, Application.Current.FindResource("LangSelectFailToAddFolderLog"), LogLevel.Feedback)
            End Try
        End Sub)
    End Sub
    ...
    Public Sub Create_Click()
        '检查是否有下载任务
        If HasDownloadingTask() Then
            Hint(Application.Current.FindResource("LangSelectDownloadingTaskExistsNoCreate"), HintType.Critical)
            Exit Sub
        End If
        If Not Directory.Exists(Path & ".minecraft\") Then
            ...
            Hint(Application.Current.FindResource("LangSelectFolderCreateSuccess"), HintType.Finish)
        End If
        McFolderListLoader.Start(IsForceRestart:=True)
    End Sub
    ...
    Public Sub Remove_Click(sender As Object, e As RoutedEventArgs)
        Try
            ...
            If Folder.Type = McFolderType.Custom Then
                Select Case MyMsgBox(Application.Current.FindResource("LangSelectDialogCleanCacheContent"), Application.Current.FindResource("LangSelectDialogCleanCacheTitle"), Application.Current.FindResource("LangDialogBtnDelete"), Application.Current.FindResource("LangDialogBtnKeep"), Application.Current.FindResource("LangDialogBtnCancel"))
                    Case 1
                        '删除配置文件
            ...
            Hint(If(Folder.Type = McFolderType.Custom, String.Format(Application.Current.FindResource("LangSelectHintFolderRemoved"), Name), Application.Current.FindResource("LangSelectHintFolderNameRestored")), HintType.Finish)
            McFolderListLoader.Start(IsForceRestart:=True)

        Catch ex As Exception
            Log(ex, Application.Current.FindResource("LangSelectRemoveFolderFail"), LogLevel.Feedback)
        End Try
    End Sub
    Public Sub Delete_Click(sender As Object, e As RoutedEventArgs)
        Dim Folder As McFolder = CType(CType(CType(sender.Parent, ContextMenu).Parent, Primitives.Popup).PlacementTarget, MyListItem).Tag
        Dim DeleteText As String = If((Folder.Type = McFolderType.Original OrElse Folder.Type = McFolderType.RenamedOriginal) AndAlso Folder.Path = Path & ".minecraft\" AndAlso McFolderList.Count = 1, Application.Current.FindResource("LangDialogBtnEmpty"), Application.Current.FindResource("LangDialogBtnDelete"))
        If MyMsgBox(String.Format(Application.Current.FindResource("LangSelectDialogDeleteConfirmContentA"), DeleteText, Folder.Path), Application.Current.FindResource("LangSelectDialogDeleteConfirmTitleA"), Application.Current.FindResource("LangDialogBtnCancel"), Application.Current.FindResource("LangDialogBtnOK"), Application.Current.FindResource("LangDialogBtnCancel")) <> 2 Then Exit Sub
        If MyMsgBox(String.Format(Application.Current.FindResource("LangSelectDialogDeleteConfirmContentB"), Folder.Path), Application.Current.FindResource("LangSelectDialogDeleteConfirmTitleA"), Application.Current.FindResource("LangDialogBtnOK") & DeleteText, Application.Current.FindResource("LangDialogBtnCancel"), IsWarn:=True) <> 1 Then Exit Sub
        '移出列表
        If Folder.Type = McFolderType.Custom Then
            Dim Folders As New List(Of String)(Setup.Get("LaunchFolders").ToString.Split("|"))
            For i = 0 To Folders.Count - 1
        ...
        RunInNewThread(Sub()
                           '删除文件夹
                           Try
                               Hint(String.Format(Application.Current.FindResource("LangSelectFolderDeleted"), Folder.Name), HintType.Info)
                               DeleteDirectory(Folder.Path)
                               If DeleteText = Application.Current.FindResource("LangDialogBtnEmpty") Then Directory.CreateDirectory(Folder.Path)
                               Hint(String.Format(Application.Current.FindResource("LangSelectFolderEmptied"), Folder.Name), HintType.Finish)
                           Catch ex As Exception
    ...
    Public Sub Rename_Click(sender As Object, e As RoutedEventArgs)
        ...
        Try
            ...
            Dim NewName As String =
                MyMsgBoxInput(Application.Current.FindResource("LangSelectDialogInputNewName"), "", Folder.Name,
                              New ObjectModel.Collection(Of Validate) From {New ValidateNullOrWhiteSpace, New ValidateLength(1, 30), New ValidateExcept({">", "|"})})
            If String.IsNullOrWhiteSpace(NewName) Then Exit Sub
            '修改自定义名
            Dim Folders As New List(Of String)(Setup.Get("LaunchFolders").ToString.Split("|"))
            ...
            Hint(Application.Current.FindResource("LangSelectHintNameUpdated") & NewName, HintType.Finish)
            '保存
            Setup.Set("LaunchFolders", Join(Folders.ToArray, "|"))
            McFolderListLoader.Start(IsForceRestart:=True)
        Catch ex As Exception
            Log(ex, Application.Current.FindResource("LangSelectFailToRename"), LogLevel.Feedback)
        End Try
    End Sub
    ...
    Public Sub Folder_Change(sender As MyListItem, e As RouteEventArgs)
        ...
        If HasDownloadingTask(True) Then
            Hint(Application.Current.FindResource("LangSelectDownloadingTaskExistsNoChange"), HintType.Critical)
            e.Handled = True
            Exit Sub
        End If
        '更换
